  ...
  try {
    ...
    if (futureSemesterName) {
      ...
      if (futureContext) {
        ...
        var futureAdded = appendStudentsFromContext(newBillingSheet, futureContext, existingIds);
        if (futureAdded > 0) {
...
function sendReregistrationLinks() {
  ...
  var qEmailCol       = queueHeaderMap[norm('Parent Email')];
  var qSendCol        = queueHeaderMap[norm('Send?')];
  var qSentCol        = queueHeaderMap[norm('Sent')];

  if (!qParentIdCol || !qSendCol || !qSentCol) {
    ui.alert('❌ Required columns not found in Reregistration Queue. Please rebuild the queue.');
    return;
  }
  ...
  var bLessonsRemainingCol = billingHeaderMap[norm('Lessons Remaining')];
  var bLastReconCol        = billingHeaderMap[norm('Last Reconciliation Date')];

  // parentId -> array of active, enrolled students with lesson data
  var parentStudentsMap = {};
  for (var i = 1; i < billingData.length; i++) {
    ...
    if (!studentId || !parentId || enrollment === 'Not enrolled') continue;
  ...
  // Find checked, unsent queue rows; group by parent
  var checkedParents = {};
  for (var r = 1; r < queueData.length; r++) {
    var sendFlag    = queueData[r][qSendCol - 1];
    var alreadySent = queueData[r][qSentCol - 1];
    if (!sendFlag || alreadySent) continue;

    var parentId    = String(queueData[r][qParentIdCol - 1] || '').trim();
    var parentFirst = String(queueData[r][qParentFirstCol - 1] || '').trim();
    var email       = String(queueData[r][qEmailCol - 1] || '').trim();
  ...
  if (parentIds.length === 0) {
    ui.alert('No checked, unsent rows found in the queue.');
    return;
  }

  var confirm = ui.alert(
    'Send Re-Registration Links',
    'This will send re-registration emails to ' + parentIds.length + ' parent(s). Continue?',
    ui.ButtonSet.YES_NO
  );
  ...
  for (var p = 0; p < parentIds.length; p++) {
    ...
    var parent   = checkedParents[parentId];
    var students = parentStudentsMap[parentId] || [];

    if (students.length === 0) {
      UtilityScriptLibrary.debugLog('sendReregistrationLinks', 'WARNING',
        'No active students found for parent', parentId, '');
      failed++;
      continue;
    }
    ...
    var body    =
      ...
      'It\'s time to register for your next block of lessons! ' +
      'Here\'s where things currently stand:\n\n' +
      studentLines + '\n' +
      'Please use the link below to submit your lesson request. ' +
      'You can register all of your students at once if you\'d like.\n\n' +
      link + '\n\n' +
    ...
    try {
      ...
      UtilityScriptLibrary.debugLog('sendReregistrationLinks', 'INFO',
        'Email sent', parentId + ' - ' + parent.email, '');
    } catch (emailError) {
      failed++;
      UtilityScriptLibrary.debugLog('sendReregistrationLinks', 'ERROR',
        'Email failed', parentId + ' - ' + parent.email, emailError.message);
    }
  }

  if (failed > 0) SpreadsheetApp.flush();

  ui.alert('✅ Done. Sent: ' + sent + (failed > 0 ? ', Failed: ' + failed : '') + '.');
}
...
function addLateRegistrationsToBillingCycle() {
  ...
  var semesterName = getCurrentSemesterFromBillingMetadata(billingCycleName);
  ...
  var cycleNameCol = metaHeaderMap[UtilityScriptLibrary.normalizeHeader('Billing Month')];
  if (!cycleNameCol) throw new Error('Billing Month column not found in Billing Metadata.');
  ...
  // Append from future semester form sheet if one exists
  var futureSemesterName = getFutureSemesterName(semesterName);
  if (futureSemesterName) {
    var futureContext = buildFutureSemesterContext(
      futureSemesterName, new Date(), billingCycleName, programConfig, prevHeaderMap
    );
    if (futureContext) {
      added += appendStudentsFromContext(billingSheet, futureContext, existingIds);
    }
  }
...
function appendReregistrationNewStudents(billingSheet, reregMap, overwroteIds, context) {
  try {
    ...
    for (var j = 0; j < toAppend.length; j++) {
      ...
      setCol('Teacher ID',         studentRow[studentHdrMap[norm('Teacher ID')] - 1]);
...
function appendStudentsFromContext(billingSheet, context, existingIds) {
  ...
  if (!formStudentIdCol) {
    UtilityScriptLibrary.debugLog('appendStudentsFromContext', 'WARNING',
      'Student ID column not found', context.semesterName, '');
    return 0;
  }
...
function buildFutureSemesterContext(futureSemesterName, customToday, billingCycleName, programConfig, prevHeaderMap) {
  try {
    var context = buildBillingContext(customToday, futureSemesterName, billingCycleName, programConfig);
    context.prevHeaderMap = prevHeaderMap || {};
    return context;
  } catch (e) {
    UtilityScriptLibrary.debugLog('buildFutureSemesterContext', 'INFO',
      'Future semester form sheet not available', futureSemesterName, e.message);
    return null;
  }
}
...
function buildReregistrationQueue() {
  var ui   = SpreadsheetApp.getUi();
  var norm = UtilityScriptLibrary.normalizeHeader;
  ...
  var queueSheet = UtilityScriptLibrary.getSheet('reregistrationQueue');
  if (!queueSheet) {
    ui.alert('❌ Reregistration Queue sheet not found in billing workbook.');
    return;
  }
  ...
  var bStudentIdCol        = billingHeaderMap[norm('Student ID')];
  var bFirstNameCol        = billingHeaderMap[norm('Student First Name')];
  var bLastNameCol         = billingHeaderMap[norm('Student Last Name')];
  var bTeacherIdCol        = billingHeaderMap[norm('Teacher ID')];
  var bEnrollmentCol       = billingHeaderMap[norm('Enrollment')];
  var bLessonsRemainingCol = billingHeaderMap[norm('Lessons Remaining')];
  var bLastReconCol        = billingHeaderMap[norm('Last Reconciliation Date')];
  var bParentIdCol         = billingHeaderMap[norm('Parent ID')];
  ...
  for (var p = 1; p < parentsData.length; p++) {
    ...
    var email = String(parentsData[p][pEmailCol - 1] || '').trim();
...
function copyStaticFieldsToBillingRow(newRow, sourceRow, context, getFn) {
  var fields = [
    ...
    "Teacher ID", "Enrollment", "Salutation", "Parent First Name",
  ...
  // When building from form, resolve Teacher ID from teacher display name
  // since the form captures teacher name, not ID
  if (getFn) {
    var teacherIdCol = context.headerMap[UtilityScriptLibrary.normalizeHeader("Teacher ID")];
    if (teacherIdCol) {
      var teacherNameIdx = getFn("Teacher");
      var teacherName = teacherNameIdx !== -1 ? String(sourceRow[teacherNameIdx] || '').trim() : '';
      if (teacherName) {
        newRow[teacherIdCol - 1] = UtilityScriptLibrary.getTeacherIdByDisplayName(teacherName);
      }
    }
  }
  ...
  if (addressCol) {
    if (getFn) {
      var street = String(sourceRow[getFn("Address Street")] || '').trim();
      var city = '';
      var zip = '';

      var cityIdx = getFn("City");
      var zipIdx = getFn("Zip");
      if (cityIdx !== -1 && zipIdx !== -1) {
        city = String(sourceRow[cityIdx] || '').trim();
        zip = String(sourceRow[zipIdx] || '').trim();
      } else {
        var cityZipIdx = getFn("CityZip");
        if (cityZipIdx !== -1 && sourceRow[cityZipIdx]) {
          var cityZipParsed = UtilityScriptLibrary.parseCityZipMessy(sourceRow[cityZipIdx]);
          city = cityZipParsed.city;
          zip = cityZipParsed.zip;
        }
      }
  ...
  try {
    ...
    if (!teacherLookupSheet) {
      UtilityScriptLibrary.debugLog('expandTeacherAttendanceSheets', 'ERROR',
        'Teacher Roster Lookup sheet not found', '', '');
      return;
    }
    ...
    if (!firstNameCol || !lastNameCol || !teacherIdCol || !rosterUrlCol || !statusCol) {
      UtilityScriptLibrary.debugLog('expandTeacherAttendanceSheets', 'ERROR',
        'Required columns not found in Teacher Roster Lookup', '', '');
      return;
    }
    ...
    for (var i = 1; i < teacherData.length; i++) {
      ...
      UtilityScriptLibrary.debugLog('expandTeacherAttendanceSheets', 'DEBUG',
        'Processing teacher', teacherLabel, '');

      try {
        var fileIdMatch = rosterUrl.match(/\/spreadsheets\/d\/([a-zA-Z0-9-_]+)/);
        if (!fileIdMatch) {
          UtilityScriptLibrary.debugLog('expandTeacherAttendanceSheets', 'WARNING',
            'Invalid roster URL', teacherLabel, '');
          continue;
        }
        ...
        UtilityScriptLibrary.debugLog('expandTeacherAttendanceSheets', 'WARNING',
          'Failed to process teacher', teacherLabel, teacherError.message);
      }
    }

    UtilityScriptLibrary.debugLog('expandTeacherAttendanceSheets', 'SUCCESS',
      'Expanded attendance sheets',
      'Processed: ' + processedCount + ', Expanded: ' + expandedCount, '');

  } catch (error) {
    UtilityScriptLibrary.debugLog('expandTeacherAttendanceSheets', 'ERROR',
      'Failed to expand attendance sheets', '', error.message);
    throw error;
  }
}
...
function getFutureSemesterName(currentSemesterName) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var metaSheet = ss.getSheetByName('Semester Metadata');
  if (!metaSheet) return null;

  var data = metaSheet.getDataRange().getValues();
  for (var i = 1; i < data.length - 1; i++) {
    if (String(data[i][0]).trim() === currentSemesterName) {
      return String(data[i + 1][0]).trim() || null;
    }
  }
  return null;
}
...
function getStudentBalancesFromBilling(billingSheet, teacherId) {
  ...
  var teacherCol        = headerMap[norm('Teacher ID')] - 1;
  ...
  for (var i = 0; i < data.length; i++) {
    var row = data[i];
    if (row[teacherCol] !== teacherId) continue;
...
function logMysteryStudents(mysteryStudents) {
  try {
    ...
    UtilityScriptLibrary.debugLog('logMysteryStudents', 'INFO',
      'Logging mystery students', 'Count: ' + mysteryStudents.length, '');
    ...
    if (!mysterySheet) {
      ...
      mysterySheet.getRange(1, 1, 1, 5).setValues([
        ['Date Found', 'Student ID', 'Student Name', 'Teacher ID', 'Notes']
      ]);
    ...
    for (var i = 0; i < mysteryStudents.length; i++) {
      ...
      mysterySheet.appendRow([
        ...
        student.teacherId  || 'Unknown',
        'Found in teacher attendance but not in billing - teacher needs to report new student'
      ]);
    }

    UtilityScriptLibrary.debugLog('logMysteryStudents', 'SUCCESS',
      'Logged mystery students', 'Count: ' + mysteryStudents.length, '');
...
function migrateTeacherDisplayNamesToIds() {
  ...
  for (var m = 1; m < metaData.length; m++) {
    ...
    try {
      var headerMap  = UtilityScriptLibrary.getHeaderMap(sheet);
      var teacherCol = headerMap[norm('Teacher')] || headerMap[norm('Teacher ID')];
      if (!teacherCol) continue;

      // Rename header from 'Teacher' to 'Teacher ID' if needed
      var headerRow = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
      if (String(headerRow[teacherCol - 1]).trim() === 'Teacher') {
        sheet.getRange(1, teacherCol).setValue('Teacher ID');
      }
...
function processTeacherAttendanceForBilling(teacherSS, targetDate) {
  ...
  if (signOff !== '') {
    ...
    for (var rowIdx = 2; rowIdx < data.length; rowIdx++) { // start at 2 to skip sign-off row
      var row = data[rowIdx];
      var studentId       = row[studentIdCol];
      ...
      // Track last reconciliation date from signed-off sheets
      if (sheetConfirmedDate) {
        if (!studentDataCurrentThrough[studentId] ||
            sheetConfirmedDate > studentDataCurrentThrough[studentId]) {
          studentDataCurrentThrough[studentId] = sheetConfirmedDate;
        }
      }
    }
  }
  ...
  return {
    studentHoursByMonth: studentHoursByMonth,
    studentDataCurrentThrough: studentDataCurrentThrough,
    stats: stats
  };
}
  ...
  try {
    ...
    if (existingSheet) {
      ...
      UtilityScriptLibrary.debugLog('processTeacherForNewAttendance', 'SUCCESS',
        ...
        teacherId + ' - ' + targetMonthName + ' (added ' + studentsAdded + ' students)', '');
      ...
      UtilityScriptLibrary.debugLog('processTeacherForNewAttendance', 'SUCCESS',
        ...
        teacherId + ' - ' + targetMonthName + ' (' + activeStudents.length + ' students)', '');
      return { created: true, studentCount: activeStudents.length };
    }

  } catch (error) {
    UtilityScriptLibrary.debugLog('processTeacherForNewAttendance', 'ERROR',
      'Error processing teacher', teacherId, error.message);
    throw error;
  }
}

function processTeacherReconciliation(teacherId, rosterUrl, currentBillingSheet, targetDate, currentSemester) {
  ...
  try {
    ...
    for (var j = 0; j < activeTeachers.length; j++) {
      ...
      try {
        ...
        // Most recent signed-off date wins per student across all teachers
        for (var studentId in attendanceResult.studentDataCurrentThrough) {
          var candidateDate = attendanceResult.studentDataCurrentThrough[studentId];
          if (!aggregatedDataCurrentThrough[studentId] ||
              candidateDate > aggregatedDataCurrentThrough[studentId]) {
            aggregatedDataCurrentThrough[studentId] = candidateDate;
          }
        }
    ...
    // PASS 2 — Write aggregated hours to billing; one write per student per month
    var billingSS = UtilityScriptLibrary.getWorkbook('billing');
    ...
    // Write last reconciliation dates to current billing sheet
    updateLastReconciliationDate(currentBillingSheet, aggregatedDataCurrentThrough);
...
function updateLastReconciliationDate(billingSheet, studentDataCurrentThrough) {
  var headerMap = UtilityScriptLibrary.getHeaderMap(billingSheet);
  var norm = UtilityScriptLibrary.normalizeHeader;

  var studentIdCol  = headerMap[norm('Student ID')];
  var lastReconCol  = headerMap[norm('Last Reconciliation Date')];

  if (!studentIdCol || !lastReconCol) {
    UtilityScriptLibrary.debugLog('updateLastReconciliationDate', 'WARNING',
      'Required columns not found in billing sheet', billingSheet.getName(), '');
    return;
  }
  ...
  var numDataRows  = lastRow - 1;
  var studentIds   = billingSheet.getRange(2, studentIdCol, numDataRows, 1).getValues();
  var existingDates = billingSheet.getRange(2, lastReconCol, numDataRows, 1).getValues();

  var updatedCount = 0;
  for (var i = 0; i < numDataRows; i++) {
    var rowStudentId = studentIds[i][0];
    if (!rowStudentId || studentDataCurrentThrough[rowStudentId] === undefined) continue;

    var newDate = studentDataCurrentThrough[rowStudentId];
    var existing = existingDates[i][0];

    // Only update if new date is more recent than what's already there
    if (!existing || !(existing instanceof Date) || newDate > existing) {
      billingSheet.getRange(i + 2, lastReconCol).setValue(newDate);
      updatedCount++;
    }
  }
  ...
  UtilityScriptLibrary.debugLog('updateLastReconciliationDate', 'INFO',
...
function verifyAndGetParentData(parentId, lastFourPhone) {
  try {
    ...
    for (var p = 1; p < packagesData.length; p++) {
      ...
      for (var l = 0; l < lessonLengths.length; l++) {
        var length = lessonLengths[l];
        var hours  = lessonCount * (length / 60);
        prices[length] = Math.round(hours * effectiveRate * 100) / 100;
      }
    ...
    return { success: true, parent: parentInfo, students: students, packages: packageData };
